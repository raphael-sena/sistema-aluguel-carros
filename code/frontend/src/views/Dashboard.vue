    ...
    <div class="pedido-lista">
      <div v-for="pedido in pedidos" :key="pedido.id" class="pedido-card">
        <div class="pedido-info">
          ...
          <p><strong>Veículo:</strong> {{ obterNomeVeiculo(pedido.veiculoId) }}</p>
          <p><strong>Período:</strong> {{ pedido.dataInicio }} até {{ pedido.dataFim }}</p>
        </div>
        <div class="pedido-status">
          <span class="status" :class="statusCor(pedido.status)">
            {{ pedido.status }}
          </span>
  ...
  setup() {
    ...
    const carregarDados = async () => {
      try {
        ...
        const veiculoRes = await api.get(`/veiculos/cliente/${clienteId}`)
        veiculos.value = veiculoRes.data.veiculos || []

        // Carregar pedidos e filtrar pelos do cliente
        const pedidosRes = await api.get('/pedido-aluguel')
        pedidos.value = pedidosRes.data.filter((p: Pedido) => p.clienteId === clienteId)
      } catch (error) {
    ...
    return { pedidos, veiculos, obterNomeVeiculo, statusCor }
  }
})
</script>
